...
from lab_utils_common_classification import (
    np,
    plt,
    dlblue,
    dlorange,
...
class plt_one_addpt_onclick:
    ...
    def __init__(self, x, y, w, b, logistic=True):
        ...
        ax.text(
            0.65, 0.8, "[Click to add data points]", size=10, transform=ax.transAxes
        )
    ...
    def calc_linear(self, event):
        ...
        for it in [1, 1, 1, 1, 1, 2, 4, 8, 16, 32, 64, 128, 256]:
            self.w, self.b, _ = gradient_descent(
                self.x.reshape(-1, 1),
                self.y.reshape(-1, 1),
                self.w.reshape(-1, 1),
                self.b,
